...
const getBooks = async (req, res) => {
  ...
  const allBooks = await BookHistoryModel.find({ user_id }).sort({
    createdAt: -1,
  });
...
const createBook = async (req, res) => {
  ...
  try {
    ...
    const newBook = await BookHistoryModel.create({
      bookTitle,
      authorName,
      issueDate,
      user_id,
...
const updateBook = async (req, res) => {
  ...
  const updateOneBook = await BookHistoryModel.findOneAndUpdate(
    { _id: id },
    {
      ...req.body,
    }
